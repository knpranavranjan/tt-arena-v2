...
export default function ManagePlayerTournamentLayout({ children }: { children: React.ReactNode }) {
  ...
  // Only the player who submitted this tournament (via "Host a Tournament")
  // can manage it — the form stamps `organizer` with the hosting player's own
  // name at creation time, so that's the real ownership signal.
  const isOwner = tournament.organizer === player.name;
  if (!isOwner) return null;

  const meta = statusMeta(tournament.status);
  const location = getEvent(tournament.eventId)?.location ?? tournament.venue;
  const basePath = `/player/tournaments/${tournament.id}`;
  const tabs = [
